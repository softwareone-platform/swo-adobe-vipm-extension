...
def test_sync_agreements_by_renewal_date(mocker, agreement_factory, dry_run):
    ...
    mocked_get_agreements_by_query.assert_called_once_with(
        mocked_mpt_client,
        'eq(status,Active)&'
        'any(parameters.fulfillment,and(eq(externalId,renewalDate),eq(displayValue,2024-11-08)))&'
        'any(parameters.fulfillment,and(eq(externalId,lastSyncDate),ne(displayValue,2024-11-09)))&'
        'select=-template,-name,-listing,-status,-authorization,-vendor,-client,-price,-licensee,-buyer,'
        '-seller,-product,-externalIds,-subscriptions.price,-subscriptions.agreement,'
