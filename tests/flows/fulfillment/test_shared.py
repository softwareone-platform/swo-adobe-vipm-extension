...
from tests.conftest import order_parameters_factory
...
def test_check_processing_template(mocker, mock_mpt_client, mock_order, template):
    ...
    check_processing_template(mock_mpt_client, mock_order, "fake_template_name")  # act
...
def test_set_customer_coterm_date_if_null(
    ...
    coterm = get_coterm_date(order)  # act
...
def test_set_customer_coterm_date_if_null_already_set(
    ...
    result = set_customer_coterm_date_if_null(mock_mpt_client, mock_adobe_client, order)
...
def test_set_processing_template_step_already_set_not_first_attempt(
    ...
    context = Context(order=order, due_date=dt.date(2025, 1, 1))
    step = StartOrderProcessing("my template")
...
def test_add_asset(
    ...
    add_asset(mock_setup_client, adobe_subscription, order, adobe_subscription)  # act
...
def test_add_asset_exists(mocker, order_factory, adobe_subscription_factory, assets_factory):
    ...
    add_asset(client, adobe_subscription, order, adobe_subscription)  # act
...
def test_create_or_update_asset_step_no_adobe_new_order(mocker, mock_adobe_client, order_factory):
    ...
    step(mocker.MagicMock(), context, mocker.MagicMock())  # act
...
def test_set_adobe_order_ids_created_parameter_ignores_empty_values(
    ...
    updated_order = set_adobe_order_ids_created_parameter(context, [None, "", "  "])
...
def test_validate_duplicate_lines_step_existing_item(
    ...
    mocked_fail.assert_called_once_with(
        mocked_client,
        context.order,
        ERR_EXISTING_ITEMS.to_dict(duplicates="ITM-1234-1234-1234-0010"),
    )
    mocked_next_step.assert_not_called()
...
def test_send_gc_mpt_notification(
    ...
    send_gc_mpt_notification(mock_mpt_client, order, ["deployment 1"])  # act
...
@freeze_time("2024-05-06")  # noqa: AAA01, AAA02
...
def test_nullify_flex_discount_param(
    ...
    NullifyFlexDiscountParam()(mock_mpt_client, context, mocked_next_step)  # act
