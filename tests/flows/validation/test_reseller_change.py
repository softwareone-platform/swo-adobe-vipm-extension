...
class TestValidateResellerChange:
    """Test class for reseller change validation scenarios."""
    ...
    def test_success(
        ...
        mock_adobe_client.reseller_change_request.return_value = (
            adobe_reseller_change_preview_factory(items=adobe_items_factory())
        )
    ...
    def test_no_lines_and_empty_transfer(
        self,
        mock_mpt_client,
        mock_adobe_client,
        order_factory,
        ...
        assert has_errors is True
        assert validated_order["error"] == {
            "id": "VIPM0043",
            "message": "When reviving a reseller during a reseller transfer a purchase is "
            "required. Please add at least one item to this order.",
        }

    def test_filters_items_with_deployment(
        self,
        mocker,
        mock_mpt_client,
        mock_adobe_client,
    ...
    def test_multiple_lines_match(
        ...
        mock_adobe_client.reseller_change_request.return_value = (
            adobe_reseller_change_preview_factory(items=adobe_items)
        )
        mock_adobe_client.create_preview_order.return_value = adobe_order_factory(
            ORDER_TYPE_PREVIEW, deployment_id=""
        )
    ...
    def test_lines_different_vendor_id(
        self,
        mock_mpt_client,
        mock_adobe_client,
        order_factory,
    ...
    def test_lines_different_line_count(
        self,
        mock_mpt_client,
        mock_adobe_client,
        order_factory,
