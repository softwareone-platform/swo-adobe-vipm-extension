...
def test_create_reseller_account(
    ...
    result = client.create_reseller_account(authorization_uk, "external_id", reseller_data)

    assert result == "a-reseller-id"
...
def test_create_customer_account(
    ...
    result = client.create_customer_account(
        authorization_uk, seller_id, "external_id", "COM", customer_data
    )

    assert result == {"customerId": "A-customer-id"}
...
def test_create_customer_account_lga(
    ...
    result = client.create_customer_account_lga(
...
def test_create_preview_order_upsize(
    ...
    result = client.create_preview_order(context)
...
def test_create_preview_order_upsize_product_not_found(
    ...
    deployment_id = "a_deployment_id"
    client, _, _ = adobe_client_factory()
    # Create an order with a product SKU unknown to the SKU mapping
    order = order_factory(
        lines=lines_factory(old_quantity=5, quantity=10), deployment_id=deployment_id
    )
    order["lines"][0]["item"]["externalIds"] = {"vendor": "NONEXISTCA"}
    adobe_subscription = adobe_subscription_factory(
        offer_id="NONEXISTCA01A12", deployment_id=deployment_id
    )
    requests_mocker.get(
        urljoin(
            settings.EXTENSION_CONFIG["ADOBE_API_BASE_URL"],
...
def test_create_preview_newlines_wo_deployment(
                    ...
                    "lineItems": [
                        {
                            ...
                            "offerId": adobe_full_sku,
                            "quantity": 5,
                            "flexDiscountCodes": ["NE3YC_MPQ_CY"],
                        },
                    ],
                    "currencyCode": "USD",
                },
    ...
    context = Context(
        ...
        upsize_lines=[],
        adobe_customer_id=customer_id,
        flex_discount_candidates={
            order["lines"][0]["id"]: [
                SimpleNamespace(code="NE3YC_MPQ_CY"),
                SimpleNamespace(code="FALLBACK"),
            ]
        },
    )
    ...
    assert context.flex_discount_selection == {1: "NE3YC_MPQ_CY"}
    assert context.flex_discount_rejections == {}
...
def test_create_new_order(
    ...
    result = client.create_new_order(
        authorization_uk, customer_id, adobe_order, deployment_id=deployment_id
    )
...
def test_create_new_order_commits_requested_confirmed_codes_only(
    mocker,
    settings,
    requests_mocker,
    adobe_client_factory,
        ...
        "lineItems": [
            {
                ...
                "quantity": 5,
                "flexDiscounts": [
                    {"code": "HELD_REUSABLE", "result": "SUCCESS"},
            ...
            },
            {
                ...
                "quantity": 1,
                "flexDiscounts": [{"code": "NOT_CONFIRMED", "result": "FAILURE"}],
            },
            {
                "extLineItemNumber": 3,
                "offerId": "65304580CA01A12",
                "quantity": 2,
                "flexDiscounts": [{"code": "AUTO_APPLIED", "result": "SUCCESS"}],
            },
        ],
    }
    requests_mocker.post(
                ...
                "lineItems": [
                    {
                        ...
                        "offerId": "65304578CA01A12",
                        "quantity": 5,
                        "flexDiscountCodes": ["REQUESTED"],
                    },
                    {"extLineItemNumber": 2, "offerId": "65304579CA01A12", "quantity": 1},
                    {"extLineItemNumber": 3, "offerId": "65304580CA01A12", "quantity": 2},
                ],
            }),
        ],
    )
    ...
    result = client.create_new_order(
        ...
        customer_id,
        preview_order,
        requested_codes={1: "REQUESTED", 2: "NOT_CONFIRMED"},
    )
...
def test_create_preview_renewal(
    ...
    result = client.create_preview_renewal(authorization_uk, customer_id)
...
def test_get_order(requests_mocker, settings, adobe_client_factory, adobe_authorizations_file):
    ...
    result = client.get_order(authorization_uk, customer_id, order_id)

    assert result == {"an": "order"}
...
def test_get_subscription(
    ...
    result = client.get_subscription(authorization_uk, customer_id, sub_id)

    assert result == {"a": "subscription"}
...
def test_preview_transfer(
    ...
    result = client.preview_transfer(authorization_uk, membership_id)

    assert result == {"a": "transfer-preview"}
...
def test_create_transfer(
    ...
    result = client.create_transfer(authorization_uk, seller_id, order_id, membership_id)

    assert result == {"tansferId": "adobe-transfer-id"}
...
def test_get_transfer(requests_mocker, settings, adobe_client_factory, adobe_authorizations_file):
    ...
    result = client.get_transfer(authorization_uk, membership_id, transfer_id)

    assert result == {"a": "transfer"}
...
def test_get_auth_token(requests_mocker, settings, mock_adobe_config, adobe_config_file):
    ...
    with freeze_time("2024-01-01 12:00:00"):
        result = client._get_auth_token(authorization)  # act

        assert isinstance(result, APIToken)
        assert result.token == "an-access-token"
        assert result.expires == dt.datetime.now(tz=dt.UTC) + dt.timedelta(seconds=83000 - 180)
        assert client._token_cache[authorization] == result
...
def test_get_adobe_client(mocker):
    ...
    adobe_client.get_adobe_client()  # act
...
def test_get_subscriptions(
    ...
    result = client.get_subscriptions(authorization_uk, customer_id)

    assert result == {"items": [{"a": "subscription"}]}
...
def test_get_customer(requests_mocker, settings, adobe_client_factory, adobe_authorizations_file):
    ...
    result = client.get_customer(authorization_uk, customer_id)

    assert result == {"a": "customer"}
...
def test_create_3yc_request(
    ...
    result = client.create_3yc_request(
        authorization_uk,
        "a-customer-id",
        quantities,
...
def test_get_orders(requests_mocker, settings, adobe_client_factory, adobe_authorizations_file):
    ...
    result = client.get_orders(authorization_uk, customer_id)
...
def test_get_orders_extra_filters(
    ...
    result = client.get_orders(authorization_uk, customer_id, filters={"extra": "filter"})
...
def test_get_returnable_orders_by_subscription_id_with_returning_orders(
    ...
    ret_order_2 = adobe_order_factory(reference_order_id="order_ok_4", order_type=ORDER_TYPE_RETURN)
    mocked_get_orders = mocker.patch.object(
        adobe_client.AdobeClient,
        "get_orders",
        return_value=[
...
def test_get_return_orders_by_external_reference(
    ...
    result = client.get_return_orders_by_external_reference(
        authorization_uk, customer_id, "returning-mpt-order-123"
    )

    assert result[order_ok_1["lineItems"][0]["offerId"][:10]] == [order_ok_1, order_ok_2]
    mocked_get_orders.assert_called_once_with(
        authorization_uk,
        customer_id,
        filters={
...
def test_get_customer_deployments_active_status(
    ...
    result = client.get_customer_deployments_active_status(authorization_uk, customer_id)
...
def test_get_flex_discounts_follows_pagination_links(
    ...
    result = client._fetch_flex_discounts(
        authorization,
        {
            "market-segment": "COM",
            "country": "US",
