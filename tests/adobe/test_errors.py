...
def test_simple_error(adobe_error_factory):
    """
    Test the AdobeError exception when the error returned by
    Adobe does not contain additional details.
    """
    error_data = adobe_error_factory("1234", "error message")
    error = AdobeError(error_data)
...
def test_detailed_error(adobe_error_factory):
    """
    Test the AdobeError exception when the error returned by
    Adobe does not contain additional details.
    """
    error_data = adobe_error_factory(
