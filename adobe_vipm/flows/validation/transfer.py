...
from adobe_vipm.flows.constants import (
    ...
    ERR_ADOBE_RESSELLER_TRANSFER_NO_LINES,
...
class AddResellerLinesToOrder(Step):
    ...
    def __call__(self, mpt_client, context, next_step):
        ...
        if order_lines_from_transfer:
            ...
            context.order = set_order_error(
                context.order, ERR_ADOBE_RESSELLER_TRANSFER_NO_LINES.to_dict()
            )
            context.validation_succeeded = False
            return
    ...
    def _get_order_lines_from_transfer(self, context: Context, mpt_client) -> list[Any]:
        ...
        for item in no_deployment_transfer_items:  # TODO:filter out expired?
            ...
            mapped_item = reseller_items_map[partial_sku]
    ...
    def _transfer_order_lines_match(order_lines: list[dict], transfer_lines: list[dict]) -> bool:
        ...
        for order, transfer in zip(order_lines, transfer_lines, strict=True):
            order_vendor_id = order["item"]["externalIds"]["vendor"]
            transfer_vendor_id = transfer["item"]["externalIds"]["vendor"]

            if order_vendor_id != transfer_vendor_id:
                return False
            if order["quantity"] != transfer["quantity"]:
                return False
            if order["oldQuantity"] != transfer["oldQuantity"]:
                return False
