...
class AgreementSyncer:  # noqa: WPS214
    ...
    def __post_init__(self):
        self._authorization_id: str = self._agreement["authorization"]["id"]
        self._seller_id: str = self._agreement["seller"]["id"]
        self._licensee_id: str = self._agreement["licensee"]["id"]
        self._adobe_customer_id: str = get_adobe_customer_id(self._agreement)
        self._customer: dict = self._get_customer_or_process_lost_customer()
        if not self._customer:
            return

        self._adobe_subscriptions: list[dict] = self._adobe_client.get_subscriptions(
    ...
    def sync(self, *, dry_run: bool, sync_prices: bool) -> None:  # noqa: C901
        ...
        if not self._customer:
            logger.info("No customer found. No sync neccessary")
            return
