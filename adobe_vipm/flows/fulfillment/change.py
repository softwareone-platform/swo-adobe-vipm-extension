...
class UpdateRenewalQuantities(Step):
    ...
    def __call__(self, client, context, next_step):
        ...
        if self.process_downsize_lines:
            self._update_downsize_lines(adobe_client, client, context)
        if self.process_upsize_lines:
            self._update_upsize_and_new_lines(adobe_client, client, context)
    ...
    def _update_downsize_lines(self, adobe_client, client, context):
        for line in context.downsize_lines:
            self._update_line(adobe_client, client, context, line)

    def _update_upsize_and_new_lines(self, adobe_client, client, context):
        for line in context.upsize_lines + context.new_lines:
            self._update_line(adobe_client, client, context, line)

    def _update_line(self, adobe_client, client, context, line):
