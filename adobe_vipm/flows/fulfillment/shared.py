...
class StartOrderProcessing(Step):
    ...
    def __init__(self, template_name):
        """
        Init start order processing step with template name.
...
class ValidateRenewalWindow(Step):
    ...
    def __init__(self, *, is_validation=False):
        """
        Init validate renewal date window.

        Args:
            is_validation (bool): if the step is run in the scope of validation or not
        """
...
class CompleteOrder(Step):
    ...
    def __init__(self, template_name):
        """
        Initialize the step with template name.
