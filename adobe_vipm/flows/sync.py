...
def _add_missing_subscriptions(
    ...
    logger.info("Checking missing subscriptions for agreement %s", agreement["id"])
...
def _get_subscriptions_for_update(
    ...
    today_date = dt.datetime.now(tz=dt.UTC).date().isoformat()
    for_update = []
    ...
    for sub in agreement["subscriptions"]:
        ...
        if adobe_subscription["status"] == AdobeStatus.SUBSCRIPTION_TERMINATED:
            ...
            if is_sku_end_of_sale(get_partial_sku(actual_sku), today_date):
                logger.info(
                    "> The subscription is End Of Sale, terminating subscription %s.",
                    subscription["id"],
                )
                terminate_subscription(
                    mpt_client,
                ...
                logger.info(
                    "> The subscription not End Of Sale, expiring subscription %s",
                    subscription["id"],
                )
                update_agreement_subscription(
                    mpt_client,
...
def sync_agreements_by_renewal_date(mpt_client: MPTClient, *, dry_run: bool) -> None:
    ...
    yesterday_every_month = (
        (today - dt.timedelta(days=1) - relativedelta(months=m)).isoformat()
        for m in range(12)
    )
    rql_query = (
        "eq(status,Active)&"
