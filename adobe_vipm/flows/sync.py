...
def _add_missing_subscriptions(
    ...
    items_map = {
        ...
        for item in mpt.get_product_items_by_skus(mpt_client, agreement["product"]["id"], skus)
    }
    offer_ids = [
    ...
    for adobe_subscription in missing_adobe_subscriptions:
        ...
        if item["terms"]["model"] == ItemTermsModel.ONE_TIME:
            ...
            mpt.create_agreement_subscription(mpt_client, subscription)
    ...
    *,
    ...
    for asset, adobe_subscription, actual_sku in assets_for_update:
        ...
        if not dry_run:
            ...
            mpt.update_asset(mpt_client, asset["id"], parameters=parameters)
        else:
...
def _update_subscriptions(
    ...
    *,
    ...
    for subscription, adobe_subscription, actual_sku in subscriptions_for_update:
        ...
        template_name = get_template_name_by_subscription(adobe_subscription)
        template = mpt.get_template_by_name(mpt_client, product_id, template_name)
...
def _get_assets_for_update(
    ...
    for asset in agreement["assets"]:
        ...
        mpt_asset = mpt.get_asset_by_id(mpt_client, asset["id"])
...
def _get_subscriptions_for_update(
    ...
    for subscription in agreement["subscriptions"]:
        ...
        mpt_subscription = mpt.get_agreement_subscription(mpt_client, subscription["id"])
...
def sync_global_customer_parameters(
    ...
    try:
        ...
        if parameters[Param.PHASE_FULFILLMENT.value]:
            mpt.update_agreement(mpt_client, agreement["id"], parameters=parameters)
    except Exception:
        logger.exception(
...
def get_one_time_skus(mpt_client: MPTClient, product_id: str, vendor_external_ids) -> set[str]:
    ...
    return {
        ...
        for item in mpt.get_product_items_by_period(
