...
class Command(AdobeBaseCommand):
        ...
        for agreement in self._get_agreements(options["agreements"]):
            ...
            for asset in self._get_assets_no_subscription_id(agreement["assets"]):
                ...
                if not adobe_subscription:
                    self.error(
                        f"Error updating asset {asset['id']}: subscription not found in Adobe"
                    )
                    continue

                self._update_mpt_asset(asset, adobe_subscription, dry_run=options["dry_run"])
    ...
    def _get_agreements(self, agreements: list[str]) -> list[dict[str, Any]]:
        ...
        return get_agreements_by_query(mpt_client, rql_query, limit=100)
        ...
        *,
        ...
        if dry_run:
            self.info(
                ...
                f"externalIds: {external_id_data} \n"
            )
        else:
            mpt_client = setup_client()
            try:
                update_asset(
                    mpt_client, asset["id"], parameters=parameters_data, data=external_id_data
                )
